...
Dazu werden die Beträge der zwei zu untersuchenden Widerstände durch Ablesen der Farbcodes, durch Widerstandsmessungen mit einem Multimeter und auf indirekte Weise in einem Schaltkreis ermittelt.
...
![Schaltkreise zur indirekten Widerstandsmessung. Links: Variante a). Rechts: Variante b). Quelle: Universität Potsdam, Institut für Physik und Astronomie, Grundpraktikum, Skript zum Versuch E1.](Bilder/Schaltkreis.png){width=15%, height=15%}

Die indirekte Bestimmung der Widerstände in einem Stromkreis erfolgt in zwei Varianten. Diese Unterscheiden sich in der Art des Schaltungsaufbaus. In Abbildung 3 ist der Unterschied zwischen den Varianten a) und b) erkennbar.
...
Mithilfe der Kabel und des Breadboards wurde der Skizze a) entsprechende Stromkreis aufgebaut:

![Aufbau auf Breadboard für Variante a) der indirekten Widerstandsmessung. Im Bild ist zudem bereits der Widerstand 1 eingebaut.](Bilder/SchaltungA.png){width=23%, height=23%}
...
Es sollen nun zwei Modelle erstellt werden, anhand derer die Widerstände der verwendeten elektrischen Widerstandsbauteile in den jeweiligen Schaltungen vorhergesagt werden können.
...
Das bedeutet allerdings auch, dass unser Messsystem und das physikalischess System nicht voneinander zu trennen sind.

## Annahmen
Sowohl für Schaltung a), als auch für Schaltung b) wird grundsätzlich (vereinfachend) angenommen, dass das Voltmeter dabei einen Widerstand von $10M\Omega$ und das Amperemeter einen Innenwiderstand von $0\Omega$ hat (vergleiche Ausführungen dazu bei der indirekten Widerstandsmessung).
...
schaltungA <- function(U0, I0){
  ...
  R3 = (1/(Rges-R1)-1/R2)**-1
  print("Widerstände in Schaltung a) in Ohm:")
  print(paste("R1: ", R1))
  print(paste("R2: ", R2))
  print(paste("R3: ", R3))
...
schaltungA(U0=5, I0=5.4e-06)
```
Der auf diese Weise ermittelte Wert für Widerstand 1 ($R_3\approx 1020000\Omega$)ist etwas besser, als der Wert aus dem Experiment ($R1a \equiv R_3 = (933300 \pm 5000)\Omega$), bei dessen Bestimmung die Wirkung von Voltmeter und Amperemeter vernachlässigt wurde.

Für Widerstand 2 sieht das etwas anders aus:
```{r}
# Testen mit gemessenen Werten für Widerstand 2
schaltungA(U0=0.15, I0=0.1503)
```
Widerstand 2 wird hier, wie in der indirekten Messung als $R1b\equiv R_3 \approx 0,9980\Omega$ ermittelt. 
Dass es hierbei eine größere Übereinstimmung gegeben hat, als für Widerstand 1 lässt sich so erklären, dass das parallel geschaltete Voltmeter neben dem $10M\Omega$-Widerstand für einen zum Widerstand parallel verlaufenden Stromfluss eine vergleichsweise größere Rolle gespielt hat, als neben dem $1\Omega$-Widerstand. 
Der Fehler ist entsprechend im letzteren Fall kleiner.

## Modell für Schaltung b)

![Modell zur Schaltung b)](Bilder/ModellB.png){width=20%, height=20%}

Die Modellparameter, Rahmenbedingungen, Grenzen, Annahmen, Vereinfachungen, Inputs und Outputs sind die gleichen wie in für Schaltung a).

Der Unterschied zu dem Modell für Schaltung a) besteht in einer anderen Formel zur Berechnung des Widerstandes R_3:
...
schaltungB <- function(U0, I13){
  ...
  # Berechnung des benötigten R
  R3 = (1/Rges-1/R2)**-1
  print("Widerstände in Schaltung b) in Ohm:")
  print(paste("R1: ", R1))
  print(paste("R2: ", R2))
  print(paste("R3: ", R3))
  print(paste("R_ges: ", Rges))
...
Ein Problem stellt aber dar, dass beim Durchführen des Experiments vergessen worden ist, die Stromstärke an der Spannungsquelle abzulesen.
Es werden stattdessen für diese Modelltest die mit den Multimetern bestimmten Werte eingesetzt.
Dabei geht allerdings der Stom, der durch das Voltmeter geflossen ist in der Berechnung unter.
Das Modell wird also falsch benutzt.

*Die Ergebnisse der folgenden Modelltests sind also nur im Kontext mit dieser Problematik genießbar*

Für Widerstand 1 gibt das Modell folgenden (falschen) Output:
```{r}
# Testen mit gemessenen Werten
schaltungB(U0=5.03, I13=5e-06)
```
Dieser Wert entspricht dann exakt dem indirekt bestimmten Widerstandswert.
Auch für Widerstand 2 gibt das Modell (falscherweise) den Messwert aus:

```{r}
schaltungB(U0=0.4, I13=0.1018)
```

## Fazit

Zusammenfassend lässt sich also sagen, dass mit den verwendeten Modellen für die Schaltungen a) und b) gute Werte für den zu untersuchenden Widerstand $R_3$ vorhergesagt werden können.
Für Schaltung a) konnten mit dem entsprechenden Modell sogar ein genauerer Wert vorhergesagt werden, als dies bei der indirekten Widerstandsmessung möglich war.
